...
const ProductCard = ({ product }) => {
  return (
    <div className="flex flex-col group cursor-pointer hover:opacity-95 transition-opacity">
    ...
    <div className="min-h-screen bg-[#0e0e0e] text-[#e5e2e1]" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      <header className="px-6 py-5 border-b border-[#1e1e1e] flex justify-between items-center max-w-[1400px] mx-auto">
        <div className="flex items-center gap-2">
           <h1 className="text-2xl font-bold tracking-widest text-[#F5C518]" style={{ fontFamily: "Playfair Display, serif" }}>SNITCH.</h1>
        </div>
        <div className="flex gap-6 items-center">
          {user ? (
            ...
            <>
              <Link to="/login" className="text-[#888] hover:text-[#f0ece4] transition-colors text-sm font-medium">Login</Link>
              <Link to="/register" className="px-5 py-2.5 bg-[#F5C518] text-[#1a1200] rounded-md text-sm font-bold hover:brightness-110 transition-all">Sign Up</Link>
            </>
          )}
        </div>
      </header>
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-16">
        ...
        <div className="mb-12">
          ...
          <p className="text-[#888] text-sm tracking-wide font-light max-w-xl leading-relaxed">
            Explore curated artifacts and exclusive items listed by our community of luxury merchants.
          </p>
        </div>
        ...
        {products && products.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 px-6 rounded-[12px] border border-dashed border-[#222] bg-[#111]">
            ...
            <h3 className="text-2xl font-bold text-[#e5e2e1] mb-2" style={{ fontFamily: "Playfair Display, serif" }}>No Artifacts Found</h3>
            <p className="text-[#666] text-sm text-center max-w-sm font-light">
              There are currently no items listed on the platform. Check back later for exclusive drops.
            </p>
          </div>
        )}
      </div>
